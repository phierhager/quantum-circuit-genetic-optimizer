...
def test_create_random_circuit_gene():
    circuit_gene = create_random_circuit_gene(NUM_QUBITS, POSSIBLE_CNOT_PAIRS, ENTANGLEMENT_CHANCE)
...
def test_mutate_circuit_gene():
    ...
    mutated_gene = mutate_circuit_gene(original_gene, POSSIBLE_CNOT_PAIRS, mutation_probability=1.0)
...
def test_mutate_gene_segment():
    segment = ('q1', 'q2')
    mutated_segment = mutate_gene_segment(segment, POSSIBLE_CNOT_PAIRS, mutation_probability=1.0)
...
def test_mutate_none_segment():
    segment = None
    mutated_segment = mutate_none_segment(POSSIBLE_CNOT_PAIRS, mutation_probability=1.0)
...
def test_mutate_segment():
    segment = ('q1', 'q2')
    mutated_segment = mutate_segment(segment, POSSIBLE_CNOT_PAIRS, mutation_probability=1.0)
